...
1. Haz un fork y crea una rama `feature/` o `fix/`.
2. Añade tests si modificas la lógica del juego.
3. Envía un pull request con descripción clara de los cambios.

## Siguiente pasos sugeridos

- Añadir `requirements.txt` con `pygame` y cualquier otra dependencia.
- Añadir un `CONTRIBUTING.md` y badges (license, Python version).
- Crear pruebas unitarias para la lógica de reparto/valor de cartas.

---

¿Quieres que cree ahora un `requirements.txt`, un `CONTRIBUTING.md` o que haga el commit y push de estos cambios al remoto? 
